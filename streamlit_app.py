        ...
        if not plot_data.empty:
            ...
            if active_cols:
                ...
                colorscale_thresholds = [
                    [0.0, '#FFFFFF'], # 0.0: 純白 (無熱度)
                    [0.1, '#F0F8FF'], # 0.1: 極淡藍 (微弱波動)
                    [0.2, '#00CED1'], # 0.2: 青綠 (資金初現 - 突破點 1)
                    [0.3, '#98FB98'], # 0.3: 嫩綠 (熱度擴散)
                    [0.4, '#FFFF00'], # 0.4: 鮮黃 (進入關注區)
                    [0.5, '#FFD700'], # 0.5: 金黃 (資金成型)
                    [0.6, '#FF8C00'], # 0.6: 亮橙 (熱度上升 - 突破點 2)
                    [0.7, '#FF4500'], # 0.7: 朱紅 (主力進場)
                    [0.8, '#FF0000'], # 0.8: 正紅 (大戶重倉 - 突破點 3)
                    [0.9, '#B22222'], # 0.9: 火磚紅 (極度緊繃)
                    [1.0, '#4B0082']  # 1.0: 靛紫 (資金巔峰)
                ]
